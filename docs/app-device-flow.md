...
- 当前名称前缀 `AIPIN`、厂商前缀 `A389` 和服务 UUID 过滤暂时关闭，因此所有有名称的广播都可能出现。
...
sequenceDiagram
    ...
    B-->>A: 服务和特征列表
    A->>A: 校验读特征和通知特征
    A->>B: subscribe(所有 Notify/Indicate)
    A->>B: read(FB11)
    D-->>B: 首个状态业务帧
    B-->>A: 状态可观察
    A->>B: write 0x01 / 0x06
...
6. 端点校验通过后按实际发现结果订阅所有已配置 Notify/Indicate 特征。
7. 订阅建立后读取 FB11，等待首个有效状态帧。
8. 首读成功后会话进入 `observable`，设备详情页显示可读取状态。
9. 会话随后通过 0x01/0x06 获取设备信息和业务状态；任意阶段失败都会记录原因，基础连接仍可用时不阻断状态页面。
...
连接、服务发现、订阅或首读异常时进入“会话已中断”。成功连接的设备会从扫描列表排除。
...
- 连接可观察后，App 主动请求 `0x01` 设备信息和 `0x06` 业务状态，并在设备详情展示设备编号、固件版本和存储空间。
