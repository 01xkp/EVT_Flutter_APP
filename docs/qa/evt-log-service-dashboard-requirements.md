...
- 仅接收 UTF-8 文本日志。App 本地快照名为 `aipin-YYYY-MM-DD-HH-MM-SS[-序号].log`；SFTP 最终文件名固定为 `aipin-YYYY-MM-DD-HH-MM-SS[-序号]-[a-f0-9]{8,32}.log`。接收器必须按最终格式校验，不能只接受数字序号后缀。
- App 在远端文件名中追加随机请求标识，避免多台手机同秒上传冲突；服务端将该最终文件名保存为 `source_filename`，内部存储路径仍使用不透明 `storage_key`。
