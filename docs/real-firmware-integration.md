...
Debug 构建会保存完整联调日志到应用支持目录的 `logs/aipin-YYYY-MM-DD.log`，单文件 20 MiB 轮转，保留最近 8 个文件（最多 160 MiB）。BLE 文件导入中的 `0x23` 原始帧会最多每 200 ms 批量落盘，公共下载目录镜像最多每 10 秒同步一次；导出、关闭和退出时强制写完并同步，避免日志 I/O 影响传输。设备详情的“实时日志”可查看当前路径和导出文件；Android Debug 还会尝试镜像到系统可访问的位置，镜像状态在该页面显示。关键日志包括扫描、连接、服务发现、订阅、MTU、写入、响应、断连和回连。结构化字段仍会脱敏设备引用，但 Debug 的 `raw_packet_hex` 保留完整收发字节，可能包含认证码、文件名和音频数据，只能用于受控联调；Release/Profile 构建不会持久化日志。
