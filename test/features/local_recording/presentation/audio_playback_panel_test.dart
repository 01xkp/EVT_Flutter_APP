...
void main() {
  ...
  testWidgets('progress advances during playback and stops while paused', (
    tester,
  ) async {
    final player = FakeAudioPlayer()
      ..playDuration = const Duration(seconds: 20);
    ...
    final runningPosition = tester.widget<Slider>(find.byType(Slider)).value;
    expect(runningPosition, greaterThan(3000));
    await tester.tap(find.byTooltip('暂停录音'));
    await tester.pump();
    await tester.pump(const Duration(seconds: 2));
    expect(tester.widget<Slider>(find.byType(Slider)).value, runningPosition);
    await tester.tap(find.byTooltip('播放录音'));
    await tester.pump();
