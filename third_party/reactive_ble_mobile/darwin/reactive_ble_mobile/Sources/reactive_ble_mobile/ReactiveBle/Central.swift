...
final class Central {
    ...
    func scanForDevices(with services: [ServiceID]?) {
        isScanning = true
        // CoreBluetooth treats `nil` as an unfiltered scan. Passing an empty
        // array is not documented as equivalent and can prevent discovery on
        // some iOS versions. EVT deliberately begins unfiltered because the
        // firmware distributes its identity between AdvData and Scan Response.
        let scanServices = services?.isEmpty == true ? nil : services
